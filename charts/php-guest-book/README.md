...
| redis.auth.enabled | bool | `false` | If `false`, disables redis auth, this is a requirement for the php-guest-book. |
| redis.enabled | bool | `true` | If `true`, deploys redis, this is a requirement for the php-guest-book. |
...
| redis.replica.replicaCount | int | `2` | Redis replica, replica count. |
